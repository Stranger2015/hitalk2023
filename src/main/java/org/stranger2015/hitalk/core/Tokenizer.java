 ...
 */
class Tokenizer extends BaseTokenizer implements Serializable {
...
//    }
     ...
     */
    public Tokenizer( BufferedReader text) {
        super(text);
            ...
            try { // the various parseInt checks will throw exceptions when parts of numbers are written illegally
                ...
                if (svala.startsWith("0")) {
                    if (svala.indexOf('b') == 1) {
                        return new Token("%d".formatted(parseLong(svala.substring(2), 2)), Tokenizer.INTEGER); // try binary
                    }
                    if (svala.indexOf('o') == 1) {
                        return new Token("%d".formatted(parseLong(svala.substring(2), 8)), Tokenizer.INTEGER); // try octal
                    }
                    if (svala.indexOf('x') == 1) {
                        return new Token("%d".formatted(parseLong(svala.substring(2), 16)), Tokenizer.INTEGER); // try hex
                    }
                }
                ...
                if (typeb != '.' && typeb != '\'') { // i.e. not float or character constant
                    ...
                    return new Token("%d".formatted(parseLong(svala)), Tokenizer.INTEGER);
                }
                ...
                if (typeb == '\'' && "0".equals(svala)) {
                    ...
                    if ((intVal = isCharacterCodeConstantToken(typec, svalc)) != -1)
                        return new Token("%d".formatted(intVal), Tokenizer.INTEGER);
                    ...
                    if (exponent == svalc.length() - 1) {             // the exponent must be signed exponent
                        ...
                        if (typeb2 == '+' || typeb2 == '-') {
                            ...
                            if (typec2 == TT_WORD){
                                ...
                                return new Token("%s.%s%s%s".formatted(svala, svalc, (char) typeb2, svalc2), Tokenizer.FLOAT);
                            }
                        }
                    }
                }
                ...
                return new Token("%s.%s".formatted(svala, svalc), Tokenizer.FLOAT);
