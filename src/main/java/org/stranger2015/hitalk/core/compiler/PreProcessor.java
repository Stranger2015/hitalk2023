...
import org.stranger2015.hitalk.core.runtime.compiler.PrologBaseVisitor;
...
public class PreProcessor extends PrologBaseVisitor <Integer> {
	private final Set<String> isPermanent = new HashSet<>();						// The permanent variables are stored here.
	private final Map<String, Integer> lastOccurrence = new HashMap<>(); 	// Last body part occurence, head and first count as one.
	private final Map<String, Integer> stackPosition = new HashMap<>(); 	// Position in the environments on the stack.
	private final Map<ParseTree, String> varNames = new HashMap<>();		// Compilation name of variables.
	...
	private final Map<String, String> internalVarNames = new HashMap<>();   // For retract: keeps the names of variables.
	private StringBuilder string = new StringBuilder();								// For retract: the Prolog string representation of the tree.
	...
	public Integer visitPlrule( PrologParser.PlruleContext ctx){
		reset();											// Reset to defaults
	...
	public Integer visitNumber(NumberContext ctx){ 
		...
		string.append(d); // The reason why it is first cast to Double is so that it always has the single digit precision.
		return null; 
	}
